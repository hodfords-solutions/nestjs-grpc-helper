...
To regenerate `microservice.proto` without building the whole SDK, use the `make:proto` command. It reuses the same
`sdk-config.json`: `name` becomes the proto package and `output` the destination directory.
...
| `outputPath` | Directory `microservice.proto` is written into. Defaults to the `output` field of the config |
